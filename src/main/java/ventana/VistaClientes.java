...
/**
 * Vista para la gestión de clientes. Permite visualizar, crear, editar y
 * eliminar clientes.
 */
public class VistaClientes extends JPanel {
	...
	// Controlador
	public VistaClientesController controller;
	...
	/**
	 * Constructor principal que recibe el controlador de clientes.
	 * 
	...
	public VistaClientes(ClienteController clienteController) {
		...
		System.out.println("Inicializando VistaClientes con controlador");
	...
	/**
	 * Constructor sin controlador (no recomendado para uso normal)
	 */
	public VistaClientes() {
		...
		System.out.println("AVISO: Se ha creado VistaClientes SIN controlador. Los datos no se cargarán.");
		JOptionPane.showMessageDialog(this,
				"Esta vista se ha inicializado sin controlador.\n"
						+ "Para visualizar datos, debe usar el constructor con parámetro ClienteController.",
				"Advertencia", JOptionPane.WARNING_MESSAGE);
	}

	/**
	 * Método para cargar datos iniciales explícitamente
	 */
	public void cargarDatosIniciales() {
		if (controller != null) {
			System.out.println("Cargando datos iniciales explícitamente...");
			controller.buscarClientes();
		} else {
			System.out.println("ERROR: No se pueden cargar datos, el controlador es NULL");
			JOptionPane.showMessageDialog(this, "No se pueden cargar los datos: controlador no inicializado", "Error",
	...
	private void configurarListenersBasicos() {
		...
		// Listener básico para buscar - carga todos los datos
		buscarButton.addActionListener(e -> {
			if (controller != null) {
				controller.buscarClientes();
			} else {
				mostrarMensaje(
						"No hay controlador configurado. Utilice el constructor con parámetro ClienteController.");
			}
		});
	}
	...
	private void configurarListeners() {
		...
		if (controller != null) {
			...
			System.err.println("ERROR: No se pueden configurar listeners completos, el controlador es NULL");
		}
	}
	...
	public void mostrarListado() {
		...
		try {
			...
			// Forzar recarga de datos al mostrar listado
			if (controller != null) {
	...
	public void mostrarClientes(List<Cliente> clientes) {
		System.out.println("Mostrando " + (clientes != null ? clientes.size() : "0") + " clientes en la tabla");

		if (clientes == null) {
			System.err.println("ERROR: Lista de clientes nula");
			mostrarMensaje("Error al cargar los datos: La lista de clientes es nula");
			return;
		}
		...
		for (Cliente cliente : clientes) {
			System.out.println("Agregando a tabla: " + cliente.getId() + " - " + cliente.getNombre());
			model.addRow(new Object[] { cliente.getId(), cliente.getNombre(), cliente.getRuc(), cliente.getTelefono(),
					cliente.getEmail(), cliente.getDireccion() });
		}

		System.out.println("Filas en tabla después de agregar: " + model.getRowCount());
	}
	...
	public Cliente obtenerClienteSeleccionado() {
		...
		// Convertir índice de fila seleccionada a índice de modelo
		// (necesario si la tabla está ordenada)
		int modelRow = clientesTable.convertRowIndexToModel(selectedRow);

		Cliente cliente = new Cliente();
		try {
			DefaultTableModel model = (DefaultTableModel) clientesTable.getModel();
			cliente.setId((String) model.getValueAt(modelRow, 0));
			cliente.setNombre((String) model.getValueAt(modelRow, 1));
			cliente.setRuc((String) model.getValueAt(modelRow, 2));
			cliente.setTelefono((String) model.getValueAt(modelRow, 3));
			cliente.setEmail((String) model.getValueAt(modelRow, 4));
			cliente.setDireccion((String) model.getValueAt(modelRow, 5));
		} catch (Exception e) {
	...
	public void setController(VistaClientesController controller) {
		...
		// Forzar carga de datos después de establecer el controlador
		if (controller != null) {
			controller.buscarClientes();
		}
	}
	...
	/**
	 * Método para compatibilidad con VistaClientesController que maneja tanto
	 ...
	 * Método adicional para compatibilidad.
	 * 
	 * @return null, ya que esta vista no tiene campo de búsqueda específico para
	 *         proveedores
	 */
	 ...
	 * Método adicional para compatibilidad.
	 * 
	 * @return null, ya que esta vista no tiene botón de búsqueda específico para
	 *         proveedores
	 */
